...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'guardar') {
        ...
        $sql = "SELECT * FROM dimensiones WHERE id_Modelo = ?";
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE dimensiones 
                    SET longitud = ?, ancho = ?, altura = ?, peso = ? 
                    WHERE id_Modelo = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssii", $longitud, $ancho, $altura, $peso, $idModeloSeleccionado);
        } else {
            // Insertar un nuevo registro
            $sql = "INSERT INTO dimensiones (id_Modelo, longitud, ancho, altura, peso) 
                    VALUES (?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("isssi", $idModeloSeleccionado, $longitud, $ancho, $altura, $peso);
        }
        $stmt->execute();
    } elseif ($accion === 'eliminar') {
        // Eliminar registro de dimensiones asociado al modelo
        $sql = "DELETE FROM dimensiones WHERE id_Modelo = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $idModeloSeleccionado);
        $stmt->execute();
    }
...
<head>
    ...
    <title>Gestionar Dimensiones - MALBIS</title>
        ...
        <?php if ($idModeloSeleccionado): ?>
            <form method="POST">
                ...
                <input type="text" id="longitud" name="longitud" 
                       value="<?php echo $registro ? $registro['longitud'] : ''; ?>" required>

                <label for="ancho">Ancho</label>
                <input type="text" id="ancho" name="ancho" 
                       value="<?php echo $registro ? $registro['ancho'] : ''; ?>" required>

                <label for="altura">Altura</label>
                <input type="text" id="altura" name="altura" 
                       value="<?php echo $registro ? $registro['altura'] : ''; ?>" required>

                <label for="peso">Peso</label>
                <input type="text" id="peso" name="peso" 
                       value="<?php echo $registro ? $registro['peso'] : ''; ?>" required>
